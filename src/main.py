...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self._set_target = QtWidgets.QPushButton(self.groupBox_water_tracker)
        self._set_target.setObjectName("pushButton_set_target")
        self.verticalLayout_4.addWidget(self._set_target)
        self._add_water = QtWidgets.QPushButton(self.groupBox_water_tracker)
        self._add_water.setObjectName("pushButton_add_water")
        self.verticalLayout_4.addWidget(self._add_water)
        ...
        self.read_database()
        self.check_target()
        self._set_target.clicked.connect(self.set_target)
        self._set_target.clicked.connect(self.read_database)
        self._add_water.clicked.connect(self.add_water)
        self._add_water.clicked.connect(self.read_database)
        self._add_water.clicked.connect(self.write_log)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self._set_target.setText(_translate("MainWindow", "Set"))
        self._add_water.setText(_translate("MainWindow", "Add"))

    def read_database(self):
        water_target_read = open("target", "r")
        water_target_data = water_target_read.read()
        water_target_read.close()
        water_count_read = open("count", "r")
        ...
        if water_count_integer >= water_target_integer:
            self._add_water.setEnabled(False)
        else:
            self._add_water.setEnabled(True)

    def check_target(self):
        target = self.spinBox_set_target.text()
        target_int = int(target)
        if target_int == 0:
            self._set_target.setEnabled(False)
        else:
            self._set_target.setEnabled(True)

    def set_target(self):
        target = self.spinBox_set_target.text()
        water_target_write = open("target", "w")
        water_target_write.write(target)
        water_target_write.flush()
        water_target_write.close()
        water_count_write = open("count", "w")
        water_count_write.write('0')
        water_count_write.flush()
        water_count_write.close()
        import os
        user_name = os.popen('echo $USER').read()
        water_log_append = open("water.log", "a")
        water_log_append.write("\n")
        water_log_append.write("------------------------------")
        water_log_append.write("\n\t")
        water_log_append.write("User is - " + user_name)
        water_log_append.write("\tTarget is - " + target)
        water_log_append.write("\n")
        water_log_append.write("------------------------------")
